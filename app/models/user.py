...
class User(BASE):
    ...
    email_confirmed = Column(Boolean, nullable=False, unique=True)
    admin = Column(Boolean, default=False, nullable=False)
    cart = relationship("Cart", backref="buyer")
    orders = relationship("Order", backref="customer")

    def add_to_cart(self, item_id, quantity):
        item_to_add = session.querry(Cart).filter_by(item_id=item_id, quantity=quantity, uid=self.id)
        try:
    ...
    def remove_from_cart(self, item_id, quantity):
        item_to_delete = session.querry(Cart).filter_by(item_id=item_id, quantity=quantity, uid=self.id).first()
        try:
